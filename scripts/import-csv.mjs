...
const csvPath = path.join(
  homeDir,
  "Downloads",
  "profissionais_antigravity_estruturado_completo.csv",
);
...
function findPhotoForName(name) {
  ...
  for (const photo of availablePhotos) {
    ...
    if (score > bestMatchScore && score >= 2) {
      // Require at least 2 words matching to be safe
...
console.log(
  `Processado. Total de profissionais lidos: ${output.length} de ${records.length} lidos.`,
);
