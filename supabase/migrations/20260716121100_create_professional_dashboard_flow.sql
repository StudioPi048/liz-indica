...
GRANT SELECT, INSERT ON public.professional_profile_change_requests TO authenticated;
GRANT UPDATE, DELETE ON public.professional_profile_change_requests TO authenticated;
...
CREATE POLICY "Professionals can submit own profile changes"
  ON public.professional_profile_change_requests
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
REVOKE EXECUTE ON FUNCTION public.link_professional_owner_by_email(UUID, TEXT)
FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.link_professional_owner_by_email(UUID, TEXT)
