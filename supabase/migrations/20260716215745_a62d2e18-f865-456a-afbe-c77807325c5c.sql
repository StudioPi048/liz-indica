...
CREATE POLICY "Public can submit professional applications"
  ON public.professional_applications FOR INSERT TO anon, authenticated
  WITH CHECK (status = 'received' AND admin_notes IS NULL AND reviewed_by IS NULL AND reviewed_at IS NULL);
